...
function updateSelfAssessmentRecordTrigger () {
  try {
    ...
    for (challenger of challengerData) {
      ...
      // 更新対象シートを更新する
      updateSelfAssessmentRecord_({ ...challenger, ...updateSheetData })
    }

    // team_modern_studyチャンネルに完了メッセージを投稿する
    let message = "モダン自己評価を学習記録に更新したよっ\n"
...
function selectUpdateSheetData_ (name) {
  ...
  const updateRowThisTime = mySheet.getLastRow() + 1
 ...
 */
function selectMidtermReviewData_ (myEmail) {
  const originalSheetValues = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(REFERENCE_SHEET.MIDTERM_REVIEW).getDataRange().getValues()
  const myColumnIndex = originalSheetValues[0].findIndex(column => column === myEmail)
  return {
